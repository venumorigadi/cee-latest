...
public class GenerateCsv extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			for (FileItem item : items) {
				 ...
				 */
				if (item.isFormField()) {
					Logger.getLogger(this.getClass()).log(Priority.INFO,
							"File Name = " + item.getFieldName() + ", Value = " + item.getString());
				} else {
					// Handle Uploaded files.
					Logger.getLogger(this.getClass()).log(Priority.INFO,
							"Field Name = " + item.getFieldName() + ", File Name = " + item.getName()
		...
		final String[] header = new String[] { "upload date", "state", "county", "permit number",
				"application description", "permit description", "permit type", "permit date", "job value",
				"job square feet", "job address", "job city", "job state", "job zip", "job subdivision",
				"job lot number", "owner", "owner address", "owner city", "owner state", "owner zip", "owner phone",
				"owner type", "owner url", "owner fax", "owner primary contact", "owner email", "bldcode", "units",
				"buildings", "ctype", "legal", "contractor", "contractor address", "contractor city",
				"contractor state", "contractor zip", "contractor phone", "contractor url", "contractor fax",
		...
		for (String appNumber : appNumbers) {
			try {
				HttpPost httppost2 = new HttpPost(
						"https://permits.sanantonio.gov/DP1/Metroplex/SanAntonio/permit/WIZ_SELECT.asp");
				nvps = new ArrayList<BasicNameValuePair>();
				nvps.add(new BasicNameValuePair("SELECT_APBLDG_APNO", appNumber));
				nvps.add(new BasicNameValuePair("SELECT_MISC_QUERYSTR",
						"%2520AND%2520APBLDG.APNO%253D%2527%2520%2520" + appNumber + "%2527"));
				nvps.add(new BasicNameValuePair("WIZ_ACTION", "ACTION_NEXT"));
				...
				Map<String, String> searchedRecord = displaySegmentsUpdate(
						source.getFirstElement(HTMLElementName.TABLE));

				HttpPost httppost3 = new HttpPost("https://webapps1.sanantonio.gov/bipi2/PermitDetail.aspx?PermitNo="
						+ appNumber + "&ptype=1004");
	...
	private static Map<String, String> displaySegmentsUpdate(Element element) {
			...
			if (HTMLElementName.TR.equals(trElement.getName())) {
				...
				for (Element tdElement : tdElements) {
					if (HTMLElementName.TD.equals(tdElement.getName())) {
						...
						if (countTd == 2
								&& (data.containsKey(headerTitle) || "Applicant/Contact".equals(headerTitle))) {
							...
							if ("job address".equals(headerTitle)) {
								...
								if (st.hasMoreTokens()) {
									...
									if (siteCity.indexOf(",") != -1) {
										...
										String siteState = siteCity
												.substring(siteCity.indexOf(", ") + 2, siteCity.indexOf(", ") + 4)
												.trim();
										data.put("job state", siteState);
										String siteZip = siteCity
												.substring(siteCity.indexOf(", ") + 5, siteCity.length()).trim();
										if (siteZip.length() > 5) {
								...
								for (Element ele : tdElement.getAllElements(StartTagType.COMMENT)) {
									String oTableNode = ele.toString()
											.substring(ele.toString().indexOf("=") + 1, ele.toString().indexOf("-->"))
											.trim();
									Source oTableNodeSource = new Source(oTableNode);
									if (oTableNodeSource.getFirstElement("contname") != null) {
										data.put("contractor", oTableNodeSource.getFirstElement("contname").getContent()
												.toString().replace("&amp;", "&").trim());
									}
									String contractorAddress = "";
									if (oTableNodeSource.getFirstElement("addr") != null) {
										contractorAddress = oTableNodeSource.getFirstElement("addr").getContent()
												.toString().trim();
									}
									if (oTableNodeSource.getFirstElement("addr2") != null) {
										contractorAddress = contractorAddress + " " + oTableNodeSource
												.getFirstElement("addr2").getContent().toString().trim();
									}
									data.put("contractor address", contractorAddress);
									if (oTableNodeSource.getFirstElement("city") != null) {
										data.put("contractor city", oTableNodeSource.getFirstElement("city")
												.getContent().toString().trim());
									}
									if (oTableNodeSource.getFirstElement("state") != null) {
										data.put("contractor state", oTableNodeSource.getFirstElement("state")
												.getContent().toString().trim());
									}
									if (oTableNodeSource.getFirstElement("zip") != null) {
										String contractorZip = oTableNodeSource.getFirstElement("zip").getContent()
												.toString().trim();
									...
									if (oTableNodeSource.getFirstElement("phone") != null) {
										data.put("contractor phone",
												oTableNodeSource.getFirstElement("phone").getContent().toString()
														.replace(")", ") ").replace("x", "").replace("X", "").trim());
									}
									if (oTableNodeSource.getFirstElement("fax") != null) {
										data.put("contractor fax",
												oTableNodeSource.getFirstElement("fax").getContent().toString()
														.replace(")", ") ").replace("x", "").replace("X", "").trim());
									}
									break;
								}
								...
								if (tdElement.getAllElements(StartTagType.COMMENT) != null) {
									Element ele = tdElement.getAllElements(StartTagType.COMMENT)
											.get(tdElement.getAllElements(StartTagType.COMMENT).size() - 1);
									String oTableNode = ele.toString()
											.substring(ele.toString().indexOf("=") + 1, ele.toString().indexOf("-->"))
											.trim();
									Source oTableNodeSource = new Source(oTableNode);
									...
									if (oTableNodeSource.getFirstElement("CNTCTFIRST".toLowerCase()) != null) {
										owner = oTableNodeSource.getFirstElement("CNTCTFIRST".toLowerCase())
												.getContent().toString().trim();
									}
									if (oTableNodeSource.getFirstElement("CNTCTLAST".toLowerCase()) != null) {
										owner = owner + " "
												+ oTableNodeSource.getFirstElement("CNTCTLAST".toLowerCase())
														.getContent().toString().trim();
									}
									if ("".equals(owner)) {
										owner = owner + oTableNodeSource.getFirstElement("CONAME".toLowerCase())
												.getContent().toString().trim();
									}
									owner = owner.replace("(UMBRELLA ACCOUT)", "").replace("(UMBRELLA ACCT)", "")
											.trim();

									data.put("owner", owner);
									String ownerAddress = "";
									if (oTableNodeSource.getFirstElement("addr1") != null) {
										ownerAddress = oTableNodeSource.getFirstElement("addr1").getContent().toString()
												.trim();
									}
									if (oTableNodeSource.getFirstElement("addr2") != null) {
										ownerAddress = ownerAddress + " " + oTableNodeSource.getFirstElement("addr2")
												.getContent().toString().trim();
									}
									data.put("owner address", ownerAddress);
									if (oTableNodeSource.getFirstElement("city") != null) {
										data.put("owner city", oTableNodeSource.getFirstElement("city").getContent()
												.toString().trim());
									}
									if (oTableNodeSource.getFirstElement("state") != null) {
										data.put("owner state", oTableNodeSource.getFirstElement("state").getContent()
												.toString().trim());
									}
									if (oTableNodeSource.getFirstElement("zip") != null) {
										String ownerZip = oTableNodeSource.getFirstElement("zip").getContent()
												.toString().trim();
									...
									if (oTableNodeSource.getFirstElement("dayphn") != null) {
										data.put("owner phone",
												oTableNodeSource.getFirstElement("dayphn").getContent().toString()
														.replace(")", ") ").replace("x", "").replace("X", "").trim());
									}
									if (oTableNodeSource.getFirstElement("fax") != null) {
										data.put("owner fax",
		...
		if ("".equals(data.get("contractor"))) {
			Logger.getLogger(GenerateCsv.class).log(Priority.INFO,
					"Contractor is empty, now looking record from Applicant/contact field.");
			...
			for (Element ele : source.getAllElements(StartTagType.COMMENT)) {
				String oTableNode = ele.toString()
						.substring(ele.toString().indexOf("=") + 1, ele.toString().indexOf("-->")).trim();
				Source oTableNodeSource = new Source(oTableNode);
				String contractor = "";
				...
				if ("".equals(contractor)) {
					if (oTableNodeSource.getFirstElement("CNTCTFIRST".toLowerCase()) != null) {
						contractor = oTableNodeSource.getFirstElement("CNTCTFIRST".toLowerCase()).getContent()
								.toString().trim();
					}
					if (oTableNodeSource.getFirstElement("CNTCTLAST".toLowerCase()) != null) {
						contractor = contractor + " " + oTableNodeSource.getFirstElement("CNTCTLAST".toLowerCase())
								.getContent().toString().trim();
					}
				}
				...
				if (oTableNodeSource.getFirstElement("city") != null) {
					data.put("contractor city",
							oTableNodeSource.getFirstElement("city").getContent().toString().trim());
				}
				if (oTableNodeSource.getFirstElement("state") != null) {
					data.put("contractor state",
							oTableNodeSource.getFirstElement("state").getContent().toString().trim());
				}
				if (oTableNodeSource.getFirstElement("zip") != null) {
				...
				if (oTableNodeSource.getFirstElement("dayphn") != null) {
					data.put("contractor phone", oTableNodeSource.getFirstElement("dayphn").getContent().toString()
							.replace(")", ") ").replace("x", "").replace("X", "").trim());
				}
				if (oTableNodeSource.getFirstElement("fax") != null) {
					data.put("contractor fax", oTableNodeSource.getFirstElement("fax").getContent().toString()
							.replace(")", ") ").replace("x", "").replace("X", "").trim());
				}
					...
					if (HTMLElementName.TD.equals(tdElement.getName())) {
						...
						if (countTd == 2 && searchedData.containsKey(headerTitle)) {
							if ("job value".equals(headerTitle)) {
								...
								Matcher jobValueNumber = p.matcher(
										tdElement.getRenderer().toString().replace(",", "").replace("$", "").trim());
								...
								Pattern p = Pattern.compile("(\\d+)");
								Matcher jobSquareFeetNumber = p.matcher(
										tdElement.getRenderer().toString().replace("$", "").replace(",", "").trim());
								if (jobSquareFeetNumber.find()) {
									BigDecimal jobSquareFeet = new BigDecimal(jobSquareFeetNumber.group(1));

									Matcher oldJobSquareFeetNumber = p
											.matcher(searchedData.get("job square feet").toString());
