...
public class TravisCountCsv extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request,
        ...
        String dateNow = formatter.format(currentDate.getTime()).toString() + "_TravisCounty" + ".csv";
        String processedFileLocation = realPath + "/" + dateNow;
        csvFile = DESTINATION_DIR_PATH + "/" + dateNow;
        String result = processTravisCountyExcelSheet(fileLocation, processedFileLocation);
    ...
    public static String processTravisCountyExcelSheet(String fileToProcessLocation, String processedFileLocation) {
        ...
        try {
            ...
            if (!fileToBeProcessed.exists()) {
                try {
                    ...
                    Logger.getLogger(TravisCountCsv.class).error("", e);
                }
            }
            writer = new CsvMapWriter(new FileWriter(fileToBeProcessed), CsvPreference.EXCEL_PREFERENCE);
        } catch (IOException e1) {
            Logger.getLogger(TravisCountCsv.class).error("", e1);
        ...
        Logger.getLogger(TravisCountCsv.class).log(Priority.INFO, "sheetLastRowNumber : " + sheetLastRowNumber);
        for (int j = 1; j <= sheetLastRowNumber; j++) {
            XSSFRow row = sheet.getRow(j);
            Logger.getLogger(TravisCountCsv.class).log(Priority.INFO, "row : " + row.getCell(0));
            if (row != null && row.getCell(0) != null) {
                ...
                if (row.getCell(15).toString() != null) {
                    data.put("permit description", row.getCell(15).toString());
                }
                data.put("permit type", "");

                data.put("permit date", "");
                if (row.getCell(2).toString() != null) {
                    data.put("permit date", row.getCell(2).toString());
                }
                data.put("job value", "");
                if (row.getCell(16).toString() != null) {
                    data.put("job value", row.getCell(16).toString());
                }
                ...
                if (row.getCell(3).toString() != null) {
                    String[] strts = row.getCell(3).toString().replace(".", "-").split("-");
                    if (strts.length > 0) {
                        jobAddress = jobAddress + strts[0];
                    }
                }
                if (row.getCell(4).toString() != null) {
                    jobAddress = jobAddress + " " + row.getCell(4).toString();
                }
                ...
                if (row.getCell(7).toString() != null) {
                    ownerName = ownerName + row.getCell(7).toString();
                }
                if (row.getCell(8).toString() != null) {
                    ownerName = ownerName + " " + row.getCell(8).toString();
                }
                ...
                if (row.getCell(1).toString() != null) {
                    data.put("contractor", row.getCell(1).toString());
                }
                data.put("contractor address", "");
                if (row.getCell(10).toString() != null) {
                    data.put("contractor address", row.getCell(10).toString());
                }
                data.put("contractor city", "");
                if (row.getCell(11).toString() != null) {
                    data.put("contractor city", row.getCell(11).toString());
                }
                data.put("contractor state", "");
                if (row.getCell(12).toString() != null) {
                    data.put("contractor state", row.getCell(12).toString());
                }
                data.put("contractor zip", "");
                if (row.getCell(13).toString() != null) {
                    data.put("contractor zip", row.getCell(13).toString().split("-")[0]);
                }
                data.put("contractor phone", "");
                if (row.getCell(14).toString() != null) {
                    data.put("contractor phone", row.getCell(14).toString());
                }
